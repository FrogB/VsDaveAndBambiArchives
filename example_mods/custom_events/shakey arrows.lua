...
function onUpdate(elapsed)
    ...
    currentBeatCustom = (songPos / 1000) * (bpm / 200)
    ...
    if  spin == true then
        for i = 0,7 do
            ...
            if value1 == 'custom' then
                setPropertyFromGroup('strumLineNotes', i, 'x', defaultNotePos[i + 1][1] + arrowMoveX * math.sin((currentBeatCustom + i*0.25) * math.pi))

                setPropertyFromGroup('strumLineNotes', i, 'y', defaultNotePos[i + 1][2] + arrowMoveY * math.cos((currentBeatCustom + i*0.25) * math.pi))
            end
        end
    end
end
